...
So, while this \`isSuperAdmin\` demarcation might not be the right approach forever, it's a good place to start.`
        ...
        inviteToken: {
            type: 'string',
            allowNull: true,
            description: 'A unique token used to verify the user\'s identity when inviting.  Expires after 1 use, or after a set amount of time has elapsed.'
        },

        inviteTokenExpiresAt: {
            type: 'number',
            allowNull: true,
            description: 'A JS timestamp (epoch ms) representing the moment when this user\'s `inviteToken` will expire (or 0 if the user currently has no such token).',
            example: 1502844074211
        },
