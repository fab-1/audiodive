            ...
            before: {
                '/*': {
                    ...
                    fn: async function (req, res, next) {
                        ...
                        const isValidHost = ['127.0.0.1',configuredBaseHostname].some(host => req.hostname === host)
                        const isDevOrProd = (sails.config.environment === 'development' || sails.config.environment === 'production')
                        if (isDevOrProd && !req.isSocket && req.method === 'GET' && !isValidHost) {
                            sails.log.info('NOT Redirecting GET request from `' + req.hostname + '` to configured expected host (`' + configuredBaseHostname + '`)...')
